...
std::string choose_vehicle(std::string option1, std::string option2) {
    ...
    std::string vehicle_choice = "";
    //we compose the sentence choosing the first vehicle in lexicographical order
    if(option1 <= option2)
        vehicle_choice += option1;
    else
        vehicle_choice += option2;
    
    return vehicle_choice + " is clearly the better choice.";
}
